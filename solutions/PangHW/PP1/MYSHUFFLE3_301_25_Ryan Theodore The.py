...
s = int(input("Number of rounds of shuffling: "))
...
noChange = [x for (i,x) in enumerate(tea) if tea[i] == oldtea[i]]
print("Number of glasses that remained in same position as before shuffling: {}".format(len(noChange)))

# Big O of FillGlasses(n): O(n)